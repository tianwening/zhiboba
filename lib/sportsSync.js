...
function isBlockedImageUrl(url) {
  return /favicon|logo|qrcode|toparr|close\.jpg|2334\.png|apple-touch-icon|avatar|\/user\/|\/user\/default\/|hupu_logo|22613-iugkjbrc-upload-1655090836640-6\.png/i.test(
    String(url),
  );
}
...
function pickJsonLdImage(html, baseUrl) {
  ...
  for (const [, content] of scripts) {
    try {
      const payload = JSON.parse(decodeHtml(content));
      ...
      for (const entry of entries) {
        ...
        if (Array.isArray(image) && typeof image[0] === "string") {
          return absoluteUrl(image[0], baseUrl);
        }
        if (typeof image?.url === "string") {
          return absoluteUrl(image.url, baseUrl);
        }
      }
    } catch {
      // Ignore malformed JSON-LD from upstream pages.
    }
  }
...
function pickLinkedArticleImage(html, baseUrl) {
  let pathname = "";
  try {
  ...
  for (const [, href, content] of anchors) {
    const linkedUrl = absoluteUrl(decodeHtml(href), baseUrl);
    if (!linkedUrl) {
      continue;
    }

    const linkedPath = new URL(linkedUrl).pathname;
    if (linkedPath !== pathname) {
      continue;
    }

    const src =
      pickAttr(content, "img", "data-src") ||
      pickAttr(content, "img", "data-original") ||
      pickAttr(content, "img", "src");
    const imageUrl = absoluteUrl(src, baseUrl);
...
function pickHupuContentImage(html, baseUrl) {
  const blocks = html.matchAll(/<div[^>]+data-hupu-node=["']image["'][^>]*>([\s\S]*?)<\/div>/gi);

  for (const [, block] of blocks) {
    const src =
      pickAttr(block, "center", "data-origin") ||
      pickAttr(block, "center", "data_url") ||
      pickAttr(block, "center", "src") ||
      pickAttr(block, "img", "data-origin") ||
      pickAttr(block, "img", "data_url") ||
...
function pickInlineImage(html, baseUrl) {
  const images = html.matchAll(/<img\b[^>]*>/gi);

  for (const [tag] of images) {
    const src =
      pickAttr(tag, "img", "data-src") ||
      pickAttr(tag, "img", "data-original") ||
      pickAttr(tag, "img", "src");
    ...
    const imageUrl = absoluteUrl(src, baseUrl);
    if (imageUrl && !isBlockedImageUrl(imageUrl) && !/loading|blank/i.test(imageUrl)) {
      return imageUrl;
    }
  }
...
function pickArticleImage(html, baseUrl) {
  if (/hupu\.com/i.test(baseUrl)) {
    return pickHupuContentImage(html, baseUrl);
  }

  return (
    pickHupuContentImage(html, baseUrl) ??
    pickLinkedArticleImage(html, baseUrl) ??
    pickMetaImage(html, baseUrl) ??
    pickJsonLdImage(html, baseUrl) ??
    pickInlineImage(html, baseUrl)
  );
}
    ...
    RSS_FEEDS.map(async (feed) => {
      ...
      const enriched = await Promise.all(
        items.map(async (item) => {
          ...
          const imageUrl = pickArticleImage(await fetchHtml(item.sourceUrl), item.sourceUrl);
...
async function clearDefaultArticleImages(pool) {
  const { rowCount } = await pool.query(
    `
      update public.sports_articles
      set image_url = null
      where image_url is not null
        and (
          image_url ilike '%22613-iugkjbrc-upload-1655090836640-6.png%'
          or image_url ilike '%hupu_logo%'
          or image_url ilike '%favicon%'
          or image_url ilike '%/user/default/%'
          or image_url ilike '%qrcode%'
          or (
            source_name = '虎扑电竞'
            and image_url not ilike '%news-editor%'
            and image_url not ilike '%hupuapp/bbs%'
          )
        )
    `,
  );
...
export async function syncSportsContent() {
  ...
  try {
    ...
    const clearedDefaultImages = await clearDefaultArticleImages(pool);
